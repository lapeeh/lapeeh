...
const authenticate = () => {
  if (username.value === 'roby' && password.value === 'Tiram@1993') {
    ...
    localStorage.setItem('lapeeh_admin_auth', 'true')
    fetchStats()
  } else {
...
const logout = () => {
  localStorage.removeItem('lapeeh_admin_auth')
...
onMounted(() => {
  if (localStorage.getItem('lapeeh_admin_auth') === 'true') {
    isAuthenticated.value = true
    fetchStats()
  }
})
      ...
      <div class="content">
        ...
        <div v-if="activeTab === 'cli'" class="tab-content">
          ...
          <div class="bg-white dark:bg-zinc-900 p-6 rounded-xl border border-zinc-200 dark:border-zinc-800 shadow-sm">
            <h3 class="text-lg font-semibold mb-4 text-zinc-900 dark:text-zinc-100">lapeeh Version Usage</h3>
